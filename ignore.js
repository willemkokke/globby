...
// Normalize a raw ignore-file line the way git does: strip a byte order mark and trailing
// whitespace. "Trailing spaces are ignored unless they are quoted with backslash" - a
// backslash-escaped trailing space is kept, still escaped, so the line can be re-fed to the
// `ignore` package without being stripped again.
const normalizeIgnoreFileLine = line => line
	.replace(/^\uFEFF/u, '')
	.replace(/((?:\\\\)*?)(\\?\s+)$/u, (_match, backslashes, whitespace) => backslashes + (whitespace.startsWith('\\') ? String.raw`\ ` : ''));
...
/**
Get the lines of every ignore file, as git reads them, each paired with the directory of the
file that declared them.

Anchoring is only meaningful relative to that directory, and the rebased patterns have already
lost it, so keep the originals for anything that has to reason about which paths a rule covers.
*/
...
Reading them before searching for the nested ones lets the search itself skip whole ignored
directories. Otherwise the recursive search for ignore files walks even the directories that
the same `.gitignore` excludes, so a large ignored directory (a mounted share, a build
output) is enumerated on every call.
*/
const getKnownIgnoreFilePaths = (patterns, normalizedOptions, gitRoot) => {
	const isGitignoreSearch = [patterns].flat().includes(GITIGNORE_FILES_PATTERN);
...
const getIgnoreFileSearchPrune = (searchPatterns, files, normalizedOptions, gitRoot) => {
	...
	// Custom ignore files can live anywhere, including beside the cwd, so only a pure
	// `.gitignore` search may treat the rules at or above the cwd as complete.
	const gitignoreOnlySearch = [searchPatterns].flat().every(pattern => pattern === GITIGNORE_FILES_PATTERN);

	return buildPrunePatternsAndGuards(getIgnoreRules(files), matcher, cwd, {gitignoreOnlySearch});
};
...
const collectIgnoreFileArtifactsAsync = async (patterns, options, includeParentIgnoreFiles) => {
	...
	const knownPaths = getKnownIgnoreFilePaths(patterns, normalizedOptions, gitRoot);
	...
	// A negation found by the pruned search can re-include a directory the prune patterns
	// skipped, hiding the ignore files inside it. Repeat the search without pruning when that
	// happens; a single unpruned pass finds everything, so once is enough.
...
const collectIgnoreFileArtifactsSync = (patterns, options, includeParentIgnoreFiles) => {
	...
	// See the async version: a rescuing negation means the pruned search may have missed files.
	if (negationsCouldRescue(getIgnoreRules(childFiles), guardNames)) {
