...
const buildIgnoreFilterResult = (options, cwd, {rules, matcher}, globalMatcher, createFilter) => {
	const finalPredicate = combinePredicate(matcher, globalMatcher);

	// Patterns fast-glob can use to skip ignored directories while traversing. The predicate below
	// stays authoritative, so this only ever needs to be safe, never exhaustive. They are returned
	// separately from `options.ignore` so they skip directory expansion and the parent-directory
	// adjustment, which would rebase them outside the scope their ignore files govern.
