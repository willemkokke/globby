...
const createNameComparer = () => {
	...
	// A negation can only re-include the excluded path itself; nothing below it can be
	// re-included once the directory is excluded. Two globs cannot be compared this way,
	// so treat them as a possible match.
	return (pattern, name) => {
...
Used after the pruned ignore-file search: a negation found by that search can re-include a
directory the prune patterns skipped, which means ignore files inside it were never discovered.
...
*/
const getRulePrune = ({pattern, directory}, {cwd, matcher, hasNegations, canSkipAtAnyDepth, gitignoreOnlySearch}) => {
	...
	const isGlob = hasGitignoreWildcards(clean);
	if (isGlob && MICROMATCH_ONLY_SYNTAX.test(clean)) {
		return undefined;
	}

	// The leading slash stops the normalizer from prefixing `**/`; the passed value already
	// encodes the depth, and an extra `**/` would un-anchor an anchored rule.
	...
	if (!clean.includes('/') && canSkipAtAnyDepth(clean)) {
		...
		return {pattern: toFastGlob(`${prefix}**/${isGlob ? clean : toLiteralPattern(clean)}`), guardName: clean};
	}

	// Otherwise fall back to the single occurrence beside the ignore file, which names a
	// concrete path that the matcher can verify directly.
	const anchoredBody = clean.replace(/^\//u, '');
	...
	// A direct child of the cwd can only be re-included by a rule at or above the cwd, and in a
	// pure gitignore search those rules are all known already. A deeper target has intermediate
	// directories whose ignore files may not have been read yet.
...
export const buildPrunePatternsAndGuards = (rules, matcher, cwd, {gitignoreOnlySearch = false} = {}) => {
